...
def HashValue2(item):
    sumV=0
    for i in range(0,len(item)):
        sumV+=i*uniqueItem.index(item[i])
    return sumV
...
def HashBaskets2(ALLitemList,bucket_size,combo_size):
    Buckets=[0]*bucket_size
    for each in ALLitemList:
        ...
        for each in eachPair:
            ...
            Buckets[HashValue2(each)%bucket_size] =  Buckets[HashValue2(each)%bucket_size]+1
    return Buckets
...
def MultiHash(totalItems,uniqueItem,support,bucket_size):
    FrequentList=[]
    ...
    while(len(FrequentList)!=0):
        ...
        Buckets2=HashBaskets2(totalItems,bucket_size,len(FrequentList[0])+1)
        BitBuckets=BitBucket(Buckets)
        BitBuckets2=BitBucket(Buckets2)
        # print Buckets
        BucketDict={}
        for ind,value in enumerate(Buckets):
            BucketDict[ind]=value
        BucketDict2={}
        for ind,value in enumerate(Buckets2):
            BucketDict2[ind]=value
        ...
        for each in TempFrequent:
            ...
            if BitBuckets[HashValue(each)%bucket_size] == True and BitBuckets2[HashValue2(each)%bucket_size] == True and CountfromALL(each,totalItems)>=support:
...
MultiHash(AllItems,uniqueItem,support,bucket_size)
